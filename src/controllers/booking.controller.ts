...
export const createBooking = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  } catch { sendError(res, 'Failed to create booking'); }
};

export const getBookings = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const filter = req.user?.role === 'ADMIN' ? {} : { userId: req.user?.id };
    const bookings = await Booking.find(filter).populate('userId', 'name email').populate('itemId', 'title price image').sort({ createdAt: -1 });
    sendSuccess(res, 'Bookings fetched', bookings);
  } catch { sendError(res, 'Failed to fetch bookings'); }
};
...
export const updateBooking = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  } catch { sendError(res, 'Failed to update booking'); }
};
